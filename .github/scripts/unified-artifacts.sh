...
package_artifacts() {
    ...
    case "$platform" in
        windows)
            ...
            ;;
        linux|macos)
            # Create tar.gz archive - use --warning=no-file-changed to ignore "file changed as we read it" warnings
            # Check if ARTIFACT_OPTS is set and use it
            if [ -n "${ARTIFACT_OPTS:-}" ]; then
                tar $ARTIFACT_OPTS -czf "$output_file" -C "$artifacts_dir" .
            else
