...
template <typename Constants> class Regtest {
public:
...
#include "delta.h"
  ...
  void InMemoryEncodeDecode(const FileSpec &source_file,
                            const FileSpec &target_file, Block *coded_data,
    ...
    process:
      ...
      case XD3_OUTPUT:
        if (encoding) {
          if (coded_data != NULL) {
            ...
            coded_data->Append(encode_stream.next_out, encode_stream.avail_out);
          }
          // Feed this data to the decoder.
          xd3_avail_input(&decode_stream, encode_stream.next_out,
                          encode_stream.avail_out);
          xd3_consume_output(&encode_stream);
          encoding = false;
        } else {
          decoded_block.Append(decode_stream.next_out, decode_stream.avail_out);
      ...
      case XD3_WINFINISH:
        if (encoding) {
          ...
          CHECK_EQ(0, CmpDifferentBlockBytesAtOffset(decoded_block, target_file,
                                                     verified_bytes));
  ...
  void MainEncodeDecode(const TmpFile &source_file, const TmpFile &target_file,
    ...
    CHECK_EQ(0,
             xd3_main_cmdline(ecmd.size() - 1, const_cast<char **>(&ecmd[0])));
    ...
    CHECK_EQ(0,
             xd3_main_cmdline(dcmd.size() - 1, const_cast<char **>(&dcmd[0])));

    CHECK_EQ(0, test_compare_files(recon_file.Name(), target_file.Name()));
  }

  // Similar to xd3_process_memory, with support for test Options.
  // Exercises xd3_process_stream.
  int TestProcessMemory(int is_encode, int (*func)(xd3_stream *),
                        const uint8_t *input, usize_t input_size,
                        const uint8_t *source, usize_t source_size,
                        uint8_t *output, usize_t *output_size,
                        usize_t output_size_max, const Options &options) {
    xd3_stream stream;
    xd3_config config;
    xd3_source src;
    int ret;
  ...
  void EncodeDecodeAPI(const FileSpec &spec0, const FileSpec &spec1,
    ...
    usize_t out_size;
    int enc_ret = TestProcessMemory(
        true, &xd3_encode_input, to.Data(), to.Size(), from.Data(), from.Size(),
        delta->Data(), &out_size, delta->Size(), options);
    CHECK_EQ(0, enc_ret);
    ...
    usize_t recon_size;
    int dec_ret = xd3_decode_memory(delta->Data(), delta->Size(), from.Data(),
                                    from.Size(), recon.Data(), &recon_size,
                                    recon.Size(), 0);
    CHECK_EQ(0, dec_ret);
  ...
  void TestModifyMutator() {
    ...
    for (size_t i = 0; i < SIZEOF_ARRAY(test_cases); i++) {
      ChangeList cl1;
      cl1.push_back(
          Change(Change::MODIFY, test_cases[i].size, test_cases[i].addr));
  ...
  void TestAddMutator() {
    ...
    struct {
      ...
      size_t expected_adds;
        ...
        {1, Constants::BLOCK_SIZE + 1,
         3 /* changed + 1st of 2nd block, last */},
    ...
    for (size_t i = 0; i < SIZEOF_ARRAY(test_cases); i++) {
      ChangeList cl1;
      cl1.push_back(
          Change(Change::ADD, test_cases[i].size, test_cases[i].addr));
      spec0.ModifyTo(ChangeListMutator(cl1), &spec1);
      ...
      Delta delta(coded);
      CHECK_EQ(test_cases[i].expected_adds, delta.AddedBytes());
    }
  }
  ...
  void TestDeleteMutator() {
    ...
    for (size_t i = 0; i < SIZEOF_ARRAY(test_cases); i++) {
      ChangeList cl1;
      cl1.push_back(
          Change(Change::DELRANGE, test_cases[i].size, test_cases[i].addr));
      spec0.ModifyTo(ChangeListMutator(cl1), &spec1);
      CHECK_EQ(spec0.Size() - test_cases[i].size, spec1.Size());
  ...
  void TestCopyMutator() {
    ...
    for (size_t i = 0; i < SIZEOF_ARRAY(test_cases); i++) {
      ChangeList cl1;
      cl1.push_back(Change(Change::COPY, test_cases[i].size, test_cases[i].from,
                           test_cases[i].to));
      spec0.ModifyTo(ChangeListMutator(cl1), &spec1);
      CHECK_EQ(spec0.Size() + test_cases[i].size, spec1.Size());
  ...
  void TestMoveMutator() {
    ...
    for (size_t i = 0; i < SIZEOF_ARRAY(test_cases); i++) {
      ChangeList cl1;
      cl1.push_back(Change(Change::MOVE, test_cases[i].size, test_cases[i].from,
                           test_cases[i].to));
      spec0.ModifyTo(ChangeListMutator(cl1), &spec1);
      CHECK_EQ(spec0.Size(), spec1.Size());
  ...
  void TestOverwriteMutator() {
    ...
    CHECK(memcmp(b0.Data() + 30, b1.Data() + 30, Constants::BLOCK_SIZE - 30) ==
          0);
    ...
    CHECK(memcmp(b0.Data() + 10, b1.Data() + 10, Constants::BLOCK_SIZE - 10) ==
          0);
  }
  ...
  void TestNonBlocking() {
    ...
    Change ct(Change::COPYOVER, 22, Constants::BLOCK_SIZE + 50,
              Constants::BLOCK_SIZE + 20);

    // This is a source match just after the block boundary, shorter
    // than the lazy target match.
    Change cs1(Change::COPYOVER, 16, Constants::BLOCK_SIZE + 51,
               Constants::BLOCK_SIZE - 1);

    // This overwrites the original source bytes.
    Change cs2(Change::MODIFY, 108, Constants::BLOCK_SIZE + 20);
  ...
  void TestCopyWindow() {
    ...
    for (int i = 1; i <= iters; i++) {
      ...
      for (int j = 0; j < nmov; j += 2) {
        cl.push_back(Change(Change::MOVE, clen, (j + 1) * clen, j * clen));
      }
  ...
  void TestHalfBlockCopy() {
    ...
    for (int stage = 0; stage < 2; stage++) {
      ...
      cl1.push_back(Change(Change::MODIFY, Constants::BLOCK_SIZE * 7,
      ...
      const int yes = Constants::BLOCK_SIZE * 8 - Constants::BLOCK_SIZE / 2;
      const int no = Constants::BLOCK_SIZE * 8 - Constants::BLOCK_SIZE / 2;
  ...
  void FourWayMergeTest(const FileSpec &spec0, const FileSpec &spec1,
                        const FileSpec &spec2, const FileSpec &spec3) {
    TmpFile f0, f1, f2, f3;
    ExtFile d01, d12, d23;
    Options options;
    ...
    CHECK_EQ(0,
             xd3_main_cmdline(mcmd.size() - 1, const_cast<char **>(&mcmd[0])));
    ...
    CHECK_EQ(0,
             xd3_main_cmdline(tcmd.size() - 1, const_cast<char **>(&tcmd[0])));
    ...
    CHECK_EQ(
        0, xd3_main_cmdline(mcmd3.size() - 1, const_cast<char **>(&mcmd3[0])));
    ...
    CHECK_EQ(
        0, xd3_main_cmdline(tcmd3.size() - 1, const_cast<char **>(&tcmd3[0])));
    // XPR(NTR "Should equal %s\n", f3.Name());
      ...
      for (; !si1.Done(); si1.Next()) {
        ...
        for (; !si2.Done(); si2.Next()) {
          ...
          for (; !si3.Done(); si3.Next()) {
            ...
            // We're only interested in three sizes, strictly
            // decreasing. */
...
template <class T> void UnitTest() {
  Regtest<T> regtest;
  TEST(TestPrintf);
