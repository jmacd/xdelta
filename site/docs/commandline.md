...
Run xdelta3 from a terminal (Command Prompt or PowerShell on Windows). Invoking
it without arguments at an interactive terminal displays help. With redirected
standard input, no-argument operation remains an encoding filter:
...
```
Run xdelta3 from a command shell (Command Prompt or PowerShell on Windows).
